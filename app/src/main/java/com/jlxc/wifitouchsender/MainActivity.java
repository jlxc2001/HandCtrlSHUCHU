...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // CrashHandler is installed in App.onCreate().
        getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        buildUi();
        loadPrefs();
        bindPad();
        applyTarget();
        setStatus("baseline 已启动：该包不含 MediaPipe，用来判断是否是 MediaPipe/native 依赖导致启动闪退。", true);
    }
    ...
    private void buildUi() {
        ...
        title.setText("WiFi 鼠标输出端 baseline");
        ...
        desc.setText("连接车机接收端后，手机屏幕会变成触控板。接收端默认端口：47220。");
        ...
        tips.setText("提示：如果连接测试成功但点击无效，问题在接收端无障碍手势；如果连接测试失败，先确认手机和车机在同一局域网，并且接收端服务正在运行。");
